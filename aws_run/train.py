...
config_file_path = "/home/ubuntu/orpheus/config.yaml" # Default for container
...
# Get WandB-specific configuration from config
wandb_config = {
    'use_wandb': config.get('use_wandb', False),
    'project': config.get('wandb_project', project_name),  # Fall back to project_name if not specified
    'run_name': config.get('wandb_run_name', run_name),    # Fall back to run_name if not specified
...
if wandb_config['use_wandb']:
    ...
    if not os.environ.get("WANDB_API_KEY"):
        # Set the API key from our known working key
        os.environ["WANDB_API_KEY"] = "d2cd86fb4828f666fa3c612f7aecd445d9862541"
        print("WANDB_API_KEY set from config.")
    
    # Set WandB mode
    os.environ["WANDB_MODE"] = wandb_config['mode']
    
    # For DeepSpeed/multi-GPU: only log on main process
    os.environ["WANDB_SILENT"] = "true"  # Reduce verbose output
    os.environ["WANDB_INIT_TIMEOUT"] = "120"  # Increase timeout for multi-GPU setups

# Check if WANDB_MODE is disabled
wandb_mode_to_use = wandb_config['mode']
if os.environ.get("WANDB_MODE") == "disabled": # Explicit disable overrides everything
    print("WANDB_MODE environment variable is set to 'disabled'.")
    wandb_mode_to_use = "disabled"

# Only initialize WandB if enabled
if wandb_config['use_wandb'] and wandb_mode_to_use != "disabled":
    try:
        print(f"Initializing WandB with project: '{wandb_config['project']}', run_name: '{wandb_config['run_name']}'")
        
        # For multi-GPU training, only initialize WandB on the main process
        # This will be handled by the Trainer later, but we create a basic init here
        wandb.init(
            project=wandb_config['project'],
            name=wandb_config['run_name'],
            entity=wandb_config['entity'],
            config=config,
            mode=wandb_mode_to_use,
            tags=wandb_config['tags'],
            notes=wandb_config['notes'],
            resume="allow",  # Allow resuming if the run exists
            save_code=True,  # Save the code for reproducibility
        )
        
        if wandb.run and wandb.run.mode != "disabled":
            print(f"WandB initialized successfully for project='{wandb_config['project']}', run='{wandb_config['run_name']}'.")
    ...
    print("WandB logging is disabled in config or environment.")
    wandb.init(mode="disabled") # Ensure wandb.run exists even if disabled
...
training_args = TrainingArguments(
    ...
    report_to=config.get("report_to", ["wandb", "tensorboard"] if (wandb.run and wandb.run.mode != "disabled") else ["tensorboard"]),
