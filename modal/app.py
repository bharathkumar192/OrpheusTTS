...
orpheus_image = (
    ...
    .env({
        ...
        # Removed CUDA_LAUNCH_BLOCKING and TORCH_USE_CUDA_DSA for performance
    })
...
app = modal.App("orpheus-tts-api-10speakers-v2-optimized") # Updated app name for clarity
...
class OrpheusTTSAPI:
    def __init__(self):
        self.web_app = fastapi.FastAPI(
            title="Orpheus TTS API - 10 Speakers (Optimized V2.1)",
            description="Text-to-Speech using Orpheus and SNAC with support for 10 Hindi speakers.",
            version="2.1.0"
        )
    ...
    def _add_api_routes(self):
        ...
        async def http_generate_speech(request: TTSRequest):
            try:
                wav_bytes, duration = self._perform_tts(request) # Call the internal method
                print(f"TTS generation completed in {duration:.2f}s")
        ...
        async def root():
            return {
                "message": "Orpheus TTS API - 10 Speakers (Optimized V2.1) is running.",
                "model_id": MODEL_ID,
    ...
    def load_all_models_and_tokenizer(self):
        ...
        # --- Prepare Orpheus Model Loading Arguments ---
        ...
        # --- Determine and set Flash Attention 2 ---
        pt_version = torch.__version__
        if tuple(map(int, pt_version.split('.')[:2])) >= (2, 0) and \
           self.device == "cuda" and torch.cuda.get_device_capability()[0] >= 8:
            try:
                import flash_attn # Test if flash_attn is installed and importable
                model_load_args["attn_implementation"] = "flash_attention_2"
                print("Attempting to load Orpheus model WITH Flash Attention 2.")
            except ImportError:
                print("Flash Attention 2 import failed or package not installed. Will use default attention.")
        else:
            print("Flash Attention 2 not applicable (PyTorch/CUDA/GPU requirements not met). Will use default attention.")

        # --- Load Orpheus Model ---
        try:
            print(f"Loading Orpheus model with args: {model_load_args}")
            self.orpheus_model = AutoModelForCausalLM.from_pretrained(ORPHEUS_CACHE_PATH, **model_load_args)

            if model_load_args.get("attn_implementation") == "flash_attention_2":
                # A simple check after load. More robust checks might involve model.config.
                if hasattr(self.orpheus_model.config, "_attn_implementation") and \
                   self.orpheus_model.config._attn_implementation == "flash_attention_2":
                    ...
                    print("Orpheus model loaded successfully WITH Flash Attention 2.")
                elif hasattr(self.orpheus_model.model, "flash_attn_func"): # Another heuristic for some models
                    self.flash_attn_available = True
                    print("Orpheus model loaded successfully WITH Flash Attention 2 (detected via heuristic).")
                else:
                    # If the arg was passed but model doesn't confirm, it might have silently ignored it.
                    print("Orpheus model loaded. Flash Attention 2 was requested, but active status is unconfirmed from model config. Assuming default attention.")
                    self.flash_attn_available = False # Be conservative
            else:
                print("Orpheus model loaded successfully with DEFAULT attention.")
                self.flash_attn_available = False


        except Exception as e_load:
            print(f"Error loading Orpheus model with current args ({model_load_args}): {e_load}")
            print("Attempting to load Orpheus model with FALLBACK configuration (default attention, float32)...")
            fallback_args = {
                "trust_remote_code": True,
                "local_files_only": True,
            ...
            # Ensure attn_implementation is not in fallback_args
            if "attn_implementation" in fallback_args: # Should not be the case here, but good practice
                del fallback_args["attn_implementation"]

            try:
                self.orpheus_model = AutoModelForCausalLM.from_pretrained(ORPHEUS_CACHE_PATH, **fallback_args)
                print("Orpheus model loaded successfully with FALLBACK configuration (default attention, float32).")
                self.flash_attn_available = False
            except Exception as e_fallback:
                print(f"CRITICAL: Failed to load Orpheus model even with fallback: {e_fallback}")
                raise
    ...
    def _adjust_audio_speed(self, waveform_tensor: torch.Tensor, speed_factor: float) -> torch.Tensor:
        ...
        try:
            ...
            return resampled_waveform.unsqueeze(0)
        except Exception as e:
        ...
        try:
            with torch.inference_mode():
                generation_params = {
                    ...
                    "do_sample": request.temperature > 0.001 and request.temperature <=1.0,
        ...
        try:
            ...
            if final_sr_to_use != current_audio_sr:
                ...
                try:
                    ...
                    final_sr_to_use = current_audio_sr
...
def cli_test_generation(
    ...
    output_file: str = "cli_test_v2_optimized_output.wav" # Updated default filename
):
    output_dir_local = "modal_test_outputs_optimized_v2" # Updated output directory
